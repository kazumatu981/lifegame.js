...
class SquareMatrix {
    ...
    constructor(args) {
        ...
        } else if(args instanceof SquareMatrix) {
    ...
    static ElementalMatrix(dimension) {
        return Array.from({length: dimension}, (row, x) =>
                Array.from({length: dimension}, (cell, y) => x==y ? 1:0));
    }

    static ZeroMatrix(dimension) {
        return Array.from({length: dimension}, (row, x) =>
                Array.from({length: dimension}, (cell, y) => 0));
    }
    ...
    matrixConstructor(matrix) {
        if(matrix.every((row)=>row.length == matrix.length)) {
            this._matrix
                = Array.from({length: matrix.length}, (vx, ix)=>
                    Array.from({length: matrix.length}, (vy, iy)=>matrix[ix][iy]));    
        } else {
